...
class Tree:
    ...
    def zgetCeiling(self, val):
        if(self.root != None):
            self.curVal = val
            self._zgetCeiling(self.root)

    def _zgetCeiling(self, node):
        if(node != None):
            self._zgetCeiling(node.left)
            if(node.value > self.curVal):
                self.zCeiling = node.value
                return
            self._zgetCeiling(node.right)
    ...
    def getFloor(self, val):
        for i in self.allValues[::-1]:
            if(i < val):
                return i


    def PopulateAllValues(self):
        if(self.root != None):
            self._PopulateAllValues(self.root)

    def _PopulateAllValues(self, node):
        if(node != None):
            self._PopulateAllValues(node.left)
            print(node.value)
            self.allValues.append(node.value)
            self._PopulateAllValues(node.right)
...
myTree.PopulateAllValues()
print(myTree.allValues)
print(myTree.getCeiling(13))
print(myTree.getFloor(13))
print('*'*20)
myTree.zgetCeiling(13)
print(myTree.zCeiling)
exit()
