...
def fIsPalindrome(sCurString):
    if sCurString == fReverseString(sCurString):
        return bool(True)
    return bool(False)
def flPopulateList(sString, iOffset):
    lList = []
    i = iOffset
    if i == 1:
        lList.append(sString[i:])
        lList.append(sString[:-i])
    if i == 2:
        lList.append(sString[:-i])
        lList.append(sString[1:-1])
        lList.append(sString[i:])
    if i == 3:
        lList.append(sString[:-i])
        lList.append(sString[1:-2])
        lList.append(sString[2:-1])
        lList.append(sString[i:])
    return lList


def fPrintCurList(lList):
    print(lList)
    for sString in lList:
        if fIsPalindrome(sString):
            print('Current String :', sString)
            print('Longest Palindrome :',fReverseString(sString))
            return
    return
...
# n-1
    del lCurStringList[:]
    lCurStringList = flPopulateList(sCurString, 1)
    fPrintCurList(lCurStringList)

    del lCurStringList[:]
    lCurStringList = flPopulateList(sCurString, 2)
    fPrintCurList(lCurStringList)
...
#        else:
#            j = 0
#            while j != iCurStringLength:
#             del lCurStringList[:]
#             j+=1
#             for i in range(iCurStringLength):
#                 lCurStringList.append(sCurString[i:-iCurStringLength+i])
#                 for sCurSubString in lCurStringList:
#                     if fIsPalindrome(sCurSubString):
#                         print("Longest Palindrome : ",fReverseString(sCurSubString))
#             return
